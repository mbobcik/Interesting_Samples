...
namespace Costella
{
    public static class Magic<T>
    {
        ...
        public static void Upsample(IUpDown o) {for (var i = 0; i < o.DestLength; i++) {var j = (i >> 1); var k = j - 1 + ((i & 1) << 1); o[i] = C13(o[k], o[j]);}}
        public static void Downsample(IUpDown o) {for (var i = 0; i < o.DestLength; i++) {var j = (i << 1); o[i] = C1331(o[j - 1], o[j], o[j + 1], o[j + 2]);}}
        public static void Resample(IFull o) {var ks = Magic.Kernels(o); for (var i = 0; i < o.DestLength; i++) {T sum = Zero; var k = ks[i]; var m = 0; for (var j = k.Min; j <= k.Max; j++, m++) sum = MultAdd(k[m], o[j], sum); o[i] = sum;}}
    ...
    public static class Magic
    {
        ...
        public interface IFull: IUpDown {double SrcOrigin {get;} double SrcSpacing {get;} double DestOrigin {get;} double DestSpacing {get;}}
        public static Ks Kernels(IFull o) {var key = Key(o); return kernels.GetOrAdd(key, k => new Lazy<Ks>(() => new Ks(k))).Value;}
        public class K {public int Min; public int Max; public double this[int i] {get {return Weights[i];}} public double[] Weights;}
        public class Ks
        {
            ...
            public K this[int i] {get {return ks[i];}} K[] ks;
        }
        public static double Weight(double x) {if (x < -1.5 || x > 1.5) return 0; if (x < -0.5) {var z = x + 1.5; return 0.5 * z * z;} if (x > 0.5) {var z = x - 1.5; return 0.5 * z * z;} return 0.75 - x * x;}
        public static double Step(double x) {if (x < -1) return 1; if (x < 0) {var z = x + 1; return 1 - 0.5 * z * z;} if (x < 1) {var z = x - 1; return 0.5 * z * z;} return 0;}
        ...
        static Magic()
        {
            Magic<byte>.C13 = (a, b) => (byte)((a + 3 * b + 2) >> 2); Magic<byte>.C1331 = (a, b, c, d) => (byte)((a + 3 * (b + c) + d + 4) >> 3);
            Magic<int>.C13 = (a, b) => ((a + 3 * b + 2) >> 2); Magic<int>.C1331 = (a, b, c, d) => ((a + 3 * (b + c) + d + 4) >> 3);
            Magic<double>.C13 = (a, b) => (a + 3 * b) / 4; Magic<double>.C1331 = (a, b, c, d) => (a + 3 * (b + c) + d) / 8; Magic<double>.Zero = 0; Magic<double>.MultAdd = (a, b, c) => a * b + c;
        }
    }
}
